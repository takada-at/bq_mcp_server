...
class ColumnSchema(BaseModel):
    ...
    fields: Optional[List['ColumnSchema']] = Field(None, description="RECORD型の場合のネストされたフィールド") # 再帰的な定義
...
class TableMetadata(BaseModel):
    ...
    full_table_id: str = Field(..., description="完全なテーブルID (project.dataset.table)")
    schema_: Optional[TableSchema] = Field(None, alias="schema", description="テーブルスキーマ") # 'schema'はBaseModelで予約語のためエイリアスを使用
    ...
    last_modified_time: Optional[datetime.datetime] = Field(None, description="最終更新日時")
    # 他に必要なメタデータがあれば追加
...
class DatasetListResponse(BaseModel):
    ...
    datasets: List[DatasetMetadata] = Field(..., description="データセットメタデータのリスト")
...
class SearchResultItem(BaseModel):
    ...
    match_location: str = Field(..., description="キーワードがマッチした場所 ('name', 'description')")
    # 必要に応じて他の情報（説明など）も追加
...
# --- キャッシュ用データ構造 ---
# キャッシュには詳細な情報（スキーマ含む）を保持する想定
class CachedData(BaseModel):
    """キャッシュに保存するデータ全体のモデル"""
    datasets: Dict[str, List[DatasetMetadata]] = Field(default_factory=dict, description="プロジェクトIDごとのデータセットリスト") # key: project_id
    tables: Dict[str, Dict[str, List[TableMetadata]]] = Field(default_factory=dict, description="プロジェクト・データセットごとのテーブルリスト（スキーマ含む）") # key1: project_id, key2: dataset_id
    last_updated: Optional[datetime.datetime] = Field(None, description="キャッシュの最終更新日時")
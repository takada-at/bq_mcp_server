...
def _search_columns(columns: List[ColumnSchema], keyword: str, project_id: str, dataset_id: str, table_id: str) -> List[SearchResultItem]:
    ...
    for column in columns:
        ...
        if column.description and lower_keyword in column.description.lower():
            ...
            if not any(r.type == 'column' and r.column_name == column.name and r.match_location == 'description' for r in results):
                 results.append(SearchResultItem(
                    type='column',
                    project_id=project_id,
                    dataset_id=dataset_id,
        ...
        # ネストされたフィールドを再帰的に検索
        if column.fields:
            results.extend(_search_columns(column.fields, keyword, project_id, dataset_id, table_id))
    ...
    for project_id, datasets_tables in cached_data.tables.items():
        for dataset_id, tables in datasets_tables.items():
            for table in tables:
                ...
                # カラムを検索 (スキーマが存在する場合)
                if table.schema_:
                    column_results = _search_columns(table.schema_.columns, keyword, project_id, dataset_id, table.table_id)
                    # 重複を避けるため、既存の結果に含まれていないものだけ追加
                    for col_res in column_results:
                        is_duplicate = any(
                            r.type == 'column' and
                            r.project_id == col_res.project_id and
                            r.dataset_id == col_res.dataset_id and
                            r.table_id == col_res.table_id and
                            r.column_name == col_res.column_name and
                            r.match_location == col_res.match_location
                            for r in results
                        )
                        if not is_duplicate:
    ...
    if search_results:
        ...
        for item in search_results:
            if item.type == 'dataset':
                print(f"- [Dataset] {item.project_id}.{item.dataset_id} (Match: {item.match_location})")
            elif item.type == 'table':
                print(f"- [Table]   {item.project_id}.{item.dataset_id}.{item.table_id} (Match: {item.match_location})")
            elif item.type == 'column':
                print(f"- [Column]  {item.project_id}.{item.dataset_id}.{item.table_id}.{item.column_name} (Match: {item.match_location})")
    else:
        print(f"キーワード '{test_keyword}' に一致するメタデータは見つかりませんでした。")

    test_keyword_2 = "time"
    search_results_2 = search_metadata(test_keyword_2)
    if search_results_2:
        print(f"\n--- キーワード '{test_keyword_2}' の検索結果 ---")
        ...
        print(f"キーワード '{test_keyword_2}' に一致するメタデータは見つかりませんでした。")
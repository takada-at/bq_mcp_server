...
class TestMarkdownConverter:
    ...
    def test_convert_tables_to_markdown_table_with_simple_schema(self):
        ...
        table = TableMetadata(
            ...
            full_table_id="test-project.test_dataset.test_table",
            description="テスト用テーブル",
            schema=TableSchema(columns=columns)  # schema_ ではなく schema を使用
        )
    ...
    def test_convert_tables_to_markdown_with_nested_fields(self):
        ...
        # ネストされたフィールドを持つカラム
        nested_fields = [
            ColumnSchema(name="street", type="STRING", mode="NULLABLE", description="通り名"),
            ColumnSchema(name="city", type="STRING", mode="NULLABLE", description="市区町村")
        ]
    ...
    def test_convert_nested_fields_to_markdown(self):
        ...
        fields = [
            ColumnSchema(name="item_id", type="INTEGER", mode="REQUIRED", description="アイテムID"),
            ColumnSchema(name="item_name", type="STRING", mode="NULLABLE", description="アイテム名")
        ]
    ...
    def test_convert_nested_fields_to_markdown_with_indentation(self):
        ...
        fields = [
            ColumnSchema(name="test_field", type="STRING", mode="NULLABLE", description="テストフィールド")
        ]
        
        # インデントレベル2でテスト
        result = MarkdownConverter._convert_nested_fields_to_markdown(fields, indent_level=2)
